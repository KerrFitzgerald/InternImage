...
class InternImage(nn.Module):
    ...
    def forward_features(self, x):
        ...
        #x = self.avgpool(x)
        #x = torch.flatten(x, 1)
        return x, level_tensors #LEVEL TENSORS ADDED BT ME FOR DECODER USE MIGHT ALSO HAVE TO REMOVE CLASSIFIER HEAD
    ...
    def forward(self, x):
        if self.use_clip_projector: # for InternImage-H/G
            ...
            x, level_tensors = self.forward_features(x)
        #x = self.head(x)
        return x, level_tensors[:-1]
